...
##--------Variables--------##
LOGFOLDER="/tmp"
LOG="$LOGFOLDER/server_setup_log.$(date +%Y%m%d_%H%M%S)"
pid=
tmpd=
DOMAIN=
EMAIL_USER=
...
web_service_setup(){
	...
	track_progress 50 "Web_server...customized" 
	#set DNS name
	...
	log $(cat $NGINX_LOC/$NGINX_CUSTOMIZED_CONFIG_FILE_LOCATION) 
	track_progress 50 "Web_Server...created"
...
certificates_setup(){
	certbot run -n --nginx --agree-tos -d $1,www.$1,mail.$1,www.mail.$1 -m  $2@$1  --redirect >> $LOG
}
item_echo(){
...
terminal_output_summery(){
	...
	item_echo "DNS records: ~/dns_emailwizard"	
}
...
then
	...
	log "Port 80 is not open, open port for this session at first"
	echo "For more information, please check out the log file as well:"
	echo "$LOG"
fi	
